...
# Generate MVP page template
# Usage: ./scripts/generate-mvp-page.sh <route> <title> <role> <api_endpoint>
...
if [ -z "$ROUTE" ] || [ -z "$TITLE" ]; then
  echo "Usage: $0 <route> <title> [role] [api_endpoint]"
  exit 1
fi

# Convert route to file path
FILE_PATH="frontend/app${ROUTE}/page.tsx"
...
export default function ${TITLE}Page() {
  ...
  const fetchData = async () => {
    ...
    setError('');
    try {
      ${API_ENDPOINT ? `const response = await apiClients.${API_ENDPOINT};
      setData(response.data || []);` : `// TODO: Implement API call
      // Mock data for MVP
      setData([
    ...
    <div>
      ...
      {error && (
        <div className="mb-4 bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded">
          ...
          <p className="text-sm mt-1">Required fields: [TODO: Add required fields]</p>
        </div>
      )}
